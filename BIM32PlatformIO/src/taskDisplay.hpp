 ...
 */
unsigned int get_brightness(unsigned int dispNum) {
    switch(config.display.brightness.method(dispNum)) {

        /* By sunrise and sunset */
        case 0:
            return weather.get_isDay() 
                ? config.display.brightness.day(dispNum) 
                : config.display.brightness.night(dispNum);

        /* By ambeint light sensor */
        case 1: {
            float bright = 0.0;
            if(config.display.lightSensor.type(dispNum) == 0) {
                bright = validate.volt(sensors.get_analog_voltage()) 
                    ? sensors.get_analog_voltage() * 30 
                    : 20;
            }
            if(config.display.lightSensor.type(dispNum) == 1) {
                bright = validate.light(sensors.get_max44009_light()) 
                    ? sensors.get_max44009_light() 
                    : 20;
            }
            if(config.display.lightSensor.type(dispNum) == 2) {
                bright = validate.light(sensors.get_bh1750_light()) 
                    ? sensors.get_bh1750_light() 
                    : 20;
            }
            bright *= (float)config.display.lightSensor.sensitivity(dispNum) / 20.0;
        ...
        case 2: {
            unsigned int morning = config.display.dayTime(dispNum, HOUR) * 60 + config.display.dayTime(dispNum, MINUTE);
            unsigned int evening = config.display.nightTime(dispNum, HOUR) * 60 + config.display.nightTime(dispNum, MINUTE);
            unsigned int cur_time = int(hour()) * 60 + minute();
            if(morning < evening) {
                if(morning <= cur_time && cur_time < evening) return config.display.brightness.day(dispNum);
            }
            else {
                if(cur_time >= morning || cur_time < evening) return config.display.brightness.day(dispNum);
            }
            return config.display.brightness.night(dispNum);
        };

        /* Constant brightness */
        case 3: return config.display.brightness.day(dispNum);
