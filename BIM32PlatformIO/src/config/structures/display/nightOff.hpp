...
struct NightOffStruct {
    private:
        unsigned int _need[DISPLAYS] = {false, false}; // Turn off display at night
    ...
    public:
        const bool need(unsigned int num) const { if(num >= DISPLAYS) return false; return _need[num] ? true : false; }
