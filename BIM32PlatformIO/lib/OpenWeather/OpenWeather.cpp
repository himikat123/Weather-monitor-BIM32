...
bool OW_Weather::getForecast(OW_current *current, OW_hourly *hourly, OW_daily *daily,
  ...
  String url = "http://api.openweathermap.org/data/2.5/onecall?lat=" + latitude + "&lon=" + longitude + "&exclude=minutely" + exclude + "&units=" + units + "&lang=" + language + "&appid=" + api_key;
                             ...
                             String units, String language, bool secure)
{
  ...
  String url = "http://api.openweathermap.org/data/2.5/forecast?lat=" + latitude + "&lon=" + longitude + "&units=" + units + "&lang=" + language + "&appid=" + api_key;
...
void OW_Weather::forecastDataSet(const char *val) {
  ...
  if (currentParent == "list") {
    ...
    if (currentKey == "3h") forecast->prec[arrayIndex] = value.toFloat();
