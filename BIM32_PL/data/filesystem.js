...
function del(){
  ...
  if(sl!=undefined){
    ...
    if(confirm(`Czy na pewno chcesz usunąć "${sl}" ?`)){
...
function opn(s){
  ...
  if(sl!=undefined){
    ...
    if(isImageFile(sl)){
      ...
      var v=wv.appendChild(document.createElement('div'));
      v.classList.add('un');
      v.appendChild(document.createTextNode("format nieobsługiwany"));
      get('loading').classList.remove('active');
    }
  }
}
...
function list(){
  ajax(`/esp/list.php?DATA=${Math.random()}`,{
    ...
    success:function(r){
      ...
      var r="<tr><th>Nazwa</th><th>Rodzaj</th><th>Rozmiar</th></tr>";
      var tbl=get("tb");
      ...
      for(d in data.fl){
        ...
        r+=`<td class="td1">plik "${name.split(".")[1].toUpperCase()}"</td>`;
