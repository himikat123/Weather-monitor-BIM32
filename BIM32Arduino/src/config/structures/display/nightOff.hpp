...
struct NightOff {
    ...
    public:
        ...
        const unsigned int from(unsigned int num, bool level) const { if(num >= DISPLAYS) return 0; return get_time(level, _from[num]); }
        const unsigned int to(unsigned int num, bool level) const { if(num >= DISPLAYS) return 0; return get_time(level, _to[num]); }
};