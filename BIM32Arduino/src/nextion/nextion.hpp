...
void Nextion::refresh() {
    ...
    if(_power) {
        ...
        if(config.display.model(DISPLAY_1) == D_NX4832T035) _NX4832T035_timeDate();
        if(config.display.model(DISPLAY_1) == D_NX4832K035) {
...
void Nextion::setDisplayRTC() {
    if(config.display.model(DISPLAY_1) == D_NX4832K035) _NX4832K035_setRTC();
}
