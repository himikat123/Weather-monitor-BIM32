...
String Narodmon::_fieldsPrepare(unsigned int fieldNum, String metrics, String mac) {
    ...
    switch(config.narodmonSend_sensors(fieldNum)) {
        ...
        case 3: { // BME280
            // Temperature
            if(config.narodmonSend_types(fieldNum) == 0 and sensors.checkTemp(sensors.get_bme280_temp(CORRECTED))) 
                fields = field + String(sensors.get_bme280_temp(CORRECTED));
            // Humidity
            if(config.narodmonSend_types(fieldNum) == 1 and sensors.checkHum(sensors.get_bme280_hum(CORRECTED)))
                fields = field + String(sensors.get_bme280_hum(CORRECTED));
            // Pressure
            if(config.narodmonSend_types(fieldNum) == 2 and sensors.checkPres(sensors.get_bme280_pres(CORRECTED)))
                fields = field + String(sensors.get_bme280_pres(CORRECTED));
        }; break;
    
        case 4: { // BMP180
            // Temperature
            if(config.narodmonSend_types(fieldNum) == 0 and sensors.checkTemp(sensors.get_bmp180_temp(CORRECTED))) 
                fields = field + String(sensors.get_bmp180_temp(CORRECTED));
            // Pressure
            if(config.narodmonSend_types(fieldNum) == 1 and sensors.checkPres(sensors.get_bmp180_pres(CORRECTED))) 
                fields = field + String(sensors.get_bmp180_pres(CORRECTED));
        }; break;

        case 5: { // SHT21
            // Temperature
            if(config.narodmonSend_types(fieldNum) == 0 and sensors.checkTemp(sensors.get_sht21_temp(CORRECTED))) 
                fields = field + String(sensors.get_sht21_temp(CORRECTED));
            // Humidity
            if(config.narodmonSend_types(fieldNum) == 1 and sensors.checkHum(sensors.get_sht21_hum(CORRECTED)))
                fields = field + String(sensors.get_sht21_hum(CORRECTED));
        }; break;

        case 6: { // DHT22
            // Temperature
            if(config.narodmonSend_types(fieldNum) == 0 and sensors.checkTemp(sensors.get_dht22_temp(CORRECTED))) 
                fields = field + String(sensors.get_dht22_temp(CORRECTED));
            // Humidity
            if(config.narodmonSend_types(fieldNum) == 1 and sensors.checkHum(sensors.get_dht22_hum(CORRECTED))) 
        ...
        case 11: // ESP32
            if(config.narodmonSend_types(fieldNum) == 0 and sensors.checkTemp(sensors.get_esp32_temp(CORRECTED)))
        ...
        case 12: { // BME680
            // Temperature
            if(config.narodmonSend_types(fieldNum) == 0 and sensors.checkTemp(sensors.get_bme680_temp(CORRECTED))) 
                fields = field + String(sensors.get_bme680_temp(CORRECTED));
            // Humidity
            if(config.narodmonSend_types(fieldNum) == 1 and sensors.checkHum(sensors.get_bme680_hum(CORRECTED)))
                fields = field + String(sensors.get_bme680_hum(CORRECTED));
            // Pressure
            if(config.narodmonSend_types(fieldNum) == 2 and sensors.checkPres(sensors.get_bme680_pres(CORRECTED)))
                fields = field + String(sensors.get_bme680_pres(CORRECTED));
            // IAQ
            if(config.narodmonSend_types(fieldNum) == 3 and sensors.checkIaq(sensors.get_bme680_iaq(CORRECTED)))
