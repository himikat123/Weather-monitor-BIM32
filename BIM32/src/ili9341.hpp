...
void ILI9341::_showBatteryLevel() {
    // TODO разделить показ и добычу данных
    int level = 0;
...
void ILI9341::_showWindDirection(int windDirection) {
    if(_prevWindDirection != windDirection) {
        ...
        ) _showImg(x, y, "/img/wind/n.jpg");
        else if(windDirection >= 22 && windDirection < 67) _showImg(x, y, "/img/wind/ne.jpg");
        else if(windDirection >= 67 && windDirection < 112) _showImg(x, y, "/img/wind/e.jpg");
        else if(windDirection >= 112 && windDirection < 157) _showImg(x, y, "/img/wind/se.jpg");
        else if(windDirection >= 157 && windDirection < 202) _showImg(x, y, "/img/wind/s.jpg");
        else if(windDirection >= 202 && windDirection < 247) _showImg(x, y, "/img/wind/sw.jpg");
        else if(windDirection >= 247 && windDirection < 292) _showImg(x, y, "/img/wind/w.jpg");
        else if(windDirection >= 292 && windDirection < 338) _showImg(x, y, "/img/wind/nw.jpg");
        else tft.fillRect(x, y, 16, 16, BG_COLOR);    
...
int ILI9341::_getTemp(unsigned int sens, unsigned int thing) {
    ...
    switch(sens) {
        ...
        case 4: temp = sensors.get_bmp180_temp(CORRECTED); break;  // temperature from BMP180
        case 5: temp = sensors.get_sht21_temp(CORRECTED); break;   // temperature from SHT21
        case 6: temp = sensors.get_dht22_temp(CORRECTED); break;   // temperature from DHT22
        case 7: temp = sensors.get_ds18b20_temp(CORRECTED); break; // temperature from DS18B20
        default: ; break;
    }
    return round(temp);
}
...
int ILI9341::_getHum(unsigned int sens, unsigned int thing) {
    ...
    switch(sens) {
        ...
        case 4: hum = sensors.get_sht21_hum(CORRECTED); break;  // humidity from SHT21
        case 5: hum = sensors.get_dht22_hum(CORRECTED); break;  // humidity from DHT22
        default: ; break;
    }
    return round(hum);
}
...
int ILI9341::_getPres(void) {
    ...
    switch(config.display_source_presOut_sens()) {
        ...
        case 4: pres = sensors.get_bmp180_pres(CORRECTED); break; // pressure outside from BMP180
        default: ; break;
    }
    return round(pres);
}