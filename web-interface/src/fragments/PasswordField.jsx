...
class PasswordField extends React.Component {
    ...
    /**
     * checks if password exists and
     * @returns password or empty string 
     */
    getPass() {
        let pass;
        try {
            if(this.props.num == 'ap') pass = this.props.config.accessPoint.pass; 
            else pass = this.props.config.network.pass[this.props.num];
        }
        catch(e) {}
     ...
     */
    PassChanged(event) {
        if(this.props.num == 'oldPass') this.props.oldPass(event.target.value);
        else if(this.props.num == 'newPass') this.props.newPass(event.target.value);
        else {
            if(this.props.num == 'ap') this.props.config.accessPoint.pass = event.target.value; 
            else this.props.config.network.pass[this.props.num] = event.target.value;
            ...
            <>
                ...
                <div className="input-group mb-3">
                    <input className="form-control" 
                        ...
                        defaultValue={this.getPass()} 
                        onChange={this.PassChanged} 
                        maxLength="32"
                    />
                    
                    <button className="input-group-text" onClick={this.showHide} title={text.get('showHide', lang)}>
